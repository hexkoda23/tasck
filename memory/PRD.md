...
## 2026-08-13 - Anthropic-primary with automatic Emergent failover (+ new key stored)
- User supplied a NEW direct Anthropic key; stored in `/app/backend/.env` `ANTHROPIC_API_KEY`. Live curl test: STILL blocked ("usage limits ... regain access 2026-09-01") - block is at the workspace/org level, so every key in that workspace inherits it. Key IS valid (models endpoint works; `claude-opus-5` available on it).
- New behaviour requested: direct Anthropic key = PRIMARY; on any error, auto-fallback to Emergent key so alignment snapshots / pitch decks / all AI flows ALWAYS return data.
- Env changes: `TASCK_AI_PROVIDER=anthropic` (was emergent); per user request set direct-path model to Claude Opus 5 via `ALIGNMENT_ANALYZER_MODEL=claude-opus-5`, `CREATOR_MATCH_LLM_MODEL=claude-opus-5`, `BRAINSTORM_LLM_MODEL=claude-opus-5` (ALIGNMENT_ANALYZER_MODEL also covers brand-about, opportunity, creative-brief, pitch-deck defaults).
- Code fix (v3_routes.py): 3 AI tools had BROKEN failover - an Anthropic HTTP exception (raise_for_status) escaped to the outer except and returned None WITHOUT trying Emergent: `_call_alignment_analysis_tool`, `_call_creator_match_tool`, `_call_brainstorm_analysis_tool`. Each now wraps the http call in its own try/except and falls through to `_call_emergent()`. (brand_about, opportunity, creative_brief, pitch_deck already had safe per-call fallback.)
- Verified live: direct call to `_call_alignment_analysis_tool` -> Anthropic tried first (400 logged) -> auto-fell back -> rich 6.2k-char result, `analysis_source=emergent:anthropic/claude-sonnet-4-5`. The moment Anthropic unblocks the workspace, all flows will use the direct key (Opus 5) automatically with zero code/env changes.

## 2026-08-13 - Three UX/reliability fixes (comments load speed, scrape first-try, email logo)
1. **Brand comments & messages slow load** - root cause: `/admin/brand-communications` and the brand-portal hook fetched ALL brands + cases + interactions AND hydrated up to 80 business cases behind a full-screen loader on EVERY visit.
   - `V1AdminBrandCommunications.js`: two-phase load (reveal after 3 core lists; hydrate bundles in background) + module `commsCache` for instant revisits. Reveal now ~0.6-1.1s.
   - `V1BrandPortalData.js` `useV1BrandPortalData`: module `portalCache` (keyed by brandId, stale-while-revalidate) + new `hydrated` flag; phased reveal so Messages renders before bundles hydrate. Bundle-dependent pages (Documents/Projects/Overview) gate on `loading || !hydrated` to avoid empty-state flash. `clearBrandPortalCache()` called on portal sign-out (V1PortalLayout).
...
## 2026-06-25 (d) - BUG: Alignment Snapshot never appeared in the brand portal
Report: "the alignment snapshot never drops, even after clicking the send to brand button" (brand portal kept showing "No Alignment Snapshot has been sent to this brand yet").
Root cause: in `POST /business-cases/{id}/ai/alignment/send`, the snapshot was correctly stamped `sent_to_brand` / `sent_to_brand_at` BEFORE queuing the email, then the post-email block overwrote it with `sent_to_brand_at = email.sent_at if delivery_status == "sent" else None` and `status = delivery_status`. Any SMTP result other than "sent" (queued, failed) therefore WIPED the stamp, and the brand-portal filter (`isAlignmentVisibleToBrand` in V1BrandPortalData.js) hides anything without it - permanently. The Strategy Snapshot send had the identical bug; the Pitch Deck send did not.
Fix:
- Both send endpoints now keep `status: "sent_to_brand"` and preserve `sent_to_brand_at`; email outcome is recorded only in `last_email_status` / `last_email_error`. Sharing with a brand is an admin action, not an SMTP outcome.
