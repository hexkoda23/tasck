...
for app in tasck-api tasck-web; do
  h="$(q containerapp revision list -g "$RG" -n "$app" --query "[?properties.active] | [-1].{h:properties.healthState,r:properties.runningState,i:properties.template.containers[0].image}" -o tsv | sed 's/\t/ /g')"
...
scale="$(q containerapp show -g "$RG" -n tasck-api --query "[properties.template.scale.minReplicas,properties.template.scale.maxReplicas]" -o tsv | sed 's/\n/\//g' | paste -sd '/' -)"
[[ "$scale" == "1/1" || "$scale" == "1" ]] && pass "API scale min/max = 1/1 (in-process jobs)" || warn "API scale min/max = $scale"
...
# Use JSON output and python to avoid \r TSV issues in env name list
envnames="$(q containerapp show -g "$RG" -n tasck-api --query "properties.template.containers[0].env[].name" -o json | "$PYTHON" -c "import sys,json; names=json.load(sys.stdin); print(' '.join(names))")"
...
c=pymongo.MongoClient(os.environ['MONGO_URL'],serverSelectionTimeoutMS=10000); print('MONGO', c.server_info()['version'], c[os.environ['DB_NAME']].users.count_documents({}))\"" 2>/dev/null \
  | grep -v '^WARNING' | sed 's/\r//g' | grep -E '^(ANTHROPIC|SERPAPI|SMTP|MONGO) ')"
...
cd="$(q containerapp show -g "$RG" -n tasck-web --query "properties.configuration.ingress.customDomains[].name" -o tsv | paste -sd ' ' -)"
