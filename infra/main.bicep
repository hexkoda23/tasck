      ...
      containers: [
        {
          ...
          args: [
            'set -euo pipefail; f=/restore/$DUMP_FILE; if [ "$MODE" = "dump" ]; then echo "dumping $SOURCE_DB -> $f"; mongodump --uri "$MONGO_URL" --db "$SOURCE_DB" --archive="$f" --gzip 2>&1 | sed -E "s#(mongodb(\+srv)?://)[^@/]+@#\1<redacted>@#g"; ls -la "$f"; echo "dump finished"; exit 0; fi; if [ "$MODE" = "dumpsource" ]; then echo "dumping SOURCE (Emergent) $SOURCE_DB -> $f"; mongodump --uri "$SOURCE_MONGO_URL" --db "$SOURCE_DB" --archive="$f" --gzip 2>&1 | sed -E "s#(mongodb(\+srv)?://)[^@/]+@#\1<redacted>@#g"; ls -la "$f"; echo "source dump finished"; exit 0; fi; test -f "$f" || { echo "missing $f"; ls -la /restore; exit 1; }; case "$f" in *.tar.gz|*.tgz) rm -rf /tmp/dump; mkdir -p /tmp/dump; tar -xzf "$f" -C /tmp/dump; test -d "/tmp/dump/$SOURCE_DB" || { echo "tarball has no $SOURCE_DB/ directory:"; ls -la /tmp/dump; exit 1; }; src="--dir=/tmp/dump" ;; *.gz) src="--archive=$f --gzip" ;; *) src="--archive=$f" ;; esac; if [ "$MODE" = "dryrun" ]; then echo "dry run of $f (no writes): namespaces for $SOURCE_DB"; mongorestore --uri "$MONGO_URL" $src --nsInclude "$SOURCE_DB.*" --nsFrom "$SOURCE_DB.*" --nsTo "$TARGET_DB.*" --dryRun -vv 2>&1 | sed -E "s#(mongodb(\+srv)?://)[^@/]+@#\1<redacted>@#g" | grep -Ei "found collection|restoring|namespace|error|failed" | head -200; echo "dry run finished"; exit 0; fi; echo "restoring $f: $SOURCE_DB -> $TARGET_DB (drop=$DROP)"; extra=""; [ "$DROP" = "true" ] && extra="--drop"; mongorestore --uri "$MONGO_URL" $src --nsFrom "$SOURCE_DB.*" --nsTo "$TARGET_DB.*" --nsInclude "$SOURCE_DB.*" --maintainInsertionOrder --numParallelCollections 2 --numInsertionWorkersPerCollection 2 $extra 2>&1 | sed -E "s#(mongodb(\+srv)?://)[^@/]+@#\1<redacted>@#g"; echo "restore finished"'
          ]
          env: [
