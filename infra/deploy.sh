...
SOURCE_RESTORE_ENABLED="${SOURCE_RESTORE_ENABLED:-false}"   # true only during the production data migration window
...
deploy_bicep() {
  ...
  az deployment group create \
    ...
    --parameters \
      ...
      enableDemoLogin="$ENABLE_DEMO_LOGIN" customDomain="$CUSTOM_DOMAIN" customDomainValidationMethod="$CUSTOM_DOMAIN_VALIDATION_METHOD" customDomainCertificateId="$CUSTOM_DOMAIN_CERT_ID" operatorIp="$OPERATOR_IP" sourceRestoreEnabled="$SOURCE_RESTORE_ENABLED" smtpHost="$SMTP_HOST" smtpPort="$SMTP_PORT" smtpFromEmail="$SMTP_FROM_EMAIL" smtpFromName="$SMTP_FROM_NAME" smtpReplyTo="$SMTP_REPLY_TO" smtpUseTls="$SMTP_USE_TLS" smtpUseSsl="$SMTP_USE_SSL" aiModel="$AI_MODEL" alignmentAnalyzerTimeoutSeconds="$ALIGNMENT_ANALYZER_TIMEOUT_SECONDS" analyzeAllHardTimeoutSeconds="$ANALYZE_ALL_HARD_TIMEOUT_SECONDS" creatorMatchTimeoutSeconds="$CREATOR_MATCH_TIMEOUT_SECONDS" \
