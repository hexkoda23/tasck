...
export const StaffRoster = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in" data-testid="staff-roster">
      <div>
        <h1 className="text-2xl font-bold text-white tracking-tight">Roster</h1>
        <p className="text-white/35 text-sm">Super Creative teams managed by TASCK</p>
      </div>
        ...
        {loading ? (
          Array(8).fill(0).map((_, i) => (
            <div key={i} className="dashboard-card overflow-hidden">
              ...
              <div className="p-5">
                <div className="skeleton h-5 w-28 mx-auto mb-2"></div>
            ...
            >
              ...
              <div className="p-5">
                <div className="text-center mb-4">
                  <h3 className="text-white font-semibold text-base">{artist.team_name}</h3>
                  <p className="text-white/40 text-sm">{artist.principal_artist}</p>
                  <span className="inline-block mt-2 text-[11px] text-[#6BFF9A] bg-[#6BFF9A]/8 px-2.5 py-0.5 rounded-full">
                    {artist.genre}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div className="bg-white/[0.03] rounded-lg p-2.5 text-center">
                    <Music className="w-3.5 h-3.5 text-white/25 mx-auto mb-1" />
                    <p className="text-white/70 text-xs font-mono">{artist.spotify_listeners}</p>
                    <p className="text-white/25 text-[10px]">Spotify</p>
                  </div>
                  <div className="bg-white/[0.03] rounded-lg p-2.5 text-center">
                    <Instagram className="w-3.5 h-3.5 text-white/25 mx-auto mb-1" />
                    <p className="text-white/70 text-xs font-mono">{artist.instagram_followers}</p>
                    <p className="text-white/25 text-[10px]">Instagram</p>
                  </div>
                </div>

                <div className="mt-3 pt-3 border-t border-white/5 flex items-center justify-between">
                  <div>
                    <span className="text-white/25 text-[10px] uppercase tracking-wider">Brand Value</span>
                    <p className="text-[#6BFF9A] text-sm font-mono">{artist.brand_value}</p>
                  </div>
                  <div className="text-right">
                    <span className="text-white/25 text-[10px] uppercase tracking-wider">Wallet</span>
                    <p className="text-white text-sm font-mono">{formatNaira(artist.wallet_balance, { compact: true })}</p>
                  </div>
                </div>
              </div>
            </div>
