...
import { CheckCircle2, ChevronDown, FileSignature, MessageSquare, RotateCcw, Sparkles } from 'lucide-react';
import V3AdminOverview from '../v3/admin/V3AdminOverview';
import { useAdminNotifications } from '../../hooks/useAdminNotifications';

// localStorage key that remembers whether the admin wants the Needs
// Attention card expanded or collapsed across reloads.
const NEEDS_ATTENTION_COLLAPSED_KEY = 'tasck_v1_admin_needs_attention_collapsed';
...
const NeedsAttentionCard = () => {
  ...
  // Retractable card: state persists across reloads via localStorage so the
  // admin's preference sticks. Defaults to expanded.
  const [collapsed, setCollapsed] = useState(() => {
    if (typeof window === 'undefined') return false;
    try { return window.localStorage.getItem(NEEDS_ATTENTION_COLLAPSED_KEY) === '1'; } catch (_) { return false; }
  });
  useEffect(() => {
    if (typeof window === 'undefined') return;
    try { window.localStorage.setItem(NEEDS_ATTENTION_COLLAPSED_KEY, collapsed ? '1' : '0'); } catch (_) { /* ignore */ }
  }, [collapsed]);
    ...
    <div className="rounded-[12px] border border-[#E8E4DB] bg-white p-5 mb-6" data-testid="v1-admin-needs-attention">
      <div className="flex items-center justify-between gap-3">
        <button
          ...
          data-testid="v1-admin-needs-attention-toggle"
        >
          ...
          <div className="min-w-0">
            ...
            <h2 className="text-[18px] font-semibold text-[#1A1A1A] truncate" style={{ fontFamily: "'Fraunces', serif" }}>
