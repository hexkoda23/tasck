  ...
  return (
    <div className="relative">
      <input
        ...
        data-testid="v1-brand-login-password"
      />
      <button
        type="button"
...
const V1BrandLogin = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF7] flex items-center justify-center p-6" data-testid="v1-brand-login">
      <form onSubmit={submit} className="w-full max-w-md v3-card p-6">
        ...
        <p className="text-[11px] text-[#8A8A8A] uppercase tracking-wider mb-1">V1 Brand Portal</p>
        <h1 className="v3-heading text-2xl mb-1" style={{ fontFamily: "'Fraunces', serif" }}>Brand Login</h1>
        <p className="text-[13px] text-[#8A8A8A] mb-6">Use the login details sent to your email by TASCK.</p>
        <div className="space-y-3">
          <input
            ...
            data-testid="v1-brand-login-email"
          />
          <PasswordInput value={form.password} onChange={(password) => setForm({ ...form, password })} />
        </div>
        {error && <p className="text-[12px] text-[#B54A37] mt-3" data-testid="v1-brand-login-error">{error}</p>}
        <button type="submit" disabled={busy || !form.email || !form.password} className="v3-btn-primary w-full mt-5 justify-center" data-testid="v1-brand-login-submit">
