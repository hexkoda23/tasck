...
import { v3AddAlignmentComment, v3AddStrategySnapshotComment, v3ApproveAlignmentAs, v3ApproveSnapshot, v3CreateInteraction, v3GetBrand, v3GetBusinessCase, v3ListBusinessCases, v3ListInteractions, v3SubmitBrandFeedback } from '../../lib/v3api';
...
export const documentGroupsFromBundles = (bundles) => { const groups = { alignment: [], strategy: [], contracts: [], reports: [] }; (bundles || []).forEach((bundle) => { const businessCase = bundleCase(bundle); if (bundle.alignment_snapshot) groups.alignment.push({ kind: 'alignment', label: 'Alignment Snapshot', snapshot: bundle.alignment_snapshot, businessCase, bundle }); if (bundle.creative_snapshot) groups.strategy.push({ kind: 'strategy', label: 'Strategy Snapshot', snapshot: bundle.creative_snapshot, businessCase, bundle }); if (bundle.contract) groups.contracts.push({ kind: 'contract', label: 'Contract', snapshot: bundle.contract, businessCase, bundle }); if (bundle.final_report) groups.reports.push({ kind: 'report', label: 'Report & Feedback', snapshot: bundle.final_report, businessCase, bundle }); }); Object.keys(groups).forEach((key) => groups[key].sort((a, b) => activityTime(b.bundle) - activityTime(a.bundle))); return groups; };
...
export const BrandIdentityCard = ({ brand, session, compact = false }) => { const logo = brandLogo(brand); return <div className={'v3-card ' + (compact ? 'p-4' : 'p-5')}><div className="flex items-start gap-4"><div className="w-16 h-16 rounded-xl border border-[#E8E4DB] bg-white overflow-hidden flex items-center justify-center shrink-0">{logo ? <img src={logo} alt={brandName(brand)} className="w-full h-full object-contain" /> : <span className="text-lg font-bold text-[#1F4A3A]">{brandName(brand).slice(0, 2).toUpperCase()}</span>}</div><div className="min-w-0"><p className="text-[11px] text-[#8A8A8A] uppercase tracking-wider">Signed-in brand</p><h2 className="v3-heading text-2xl break-words" style={{ fontFamily: "'Fraunces', serif" }}>{brandName(brand)}</h2><p className="text-[13px] text-[#6B6258] mt-1">{brandIndustry(brand)} · {brandContact(brand, session)}</p>{!compact && <p className="text-[13px] text-[#5C5C5C] mt-3 leading-6">{brandAbout(brand)}</p>}</div></div></div>; };
