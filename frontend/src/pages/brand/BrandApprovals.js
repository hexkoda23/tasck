...
const getStatusStyle = (status) => {
  const styles = {
    pending: { bg: 'bg-[#FFA502]/20', text: 'text-[#FFA502]', label: 'Pending Review' },
    approved: { bg: 'bg-[#6BFF9A]/20', text: 'text-[#6BFF9A]', label: 'Approved' },
    rejected: { bg: 'bg-[#FF4757]/20', text: 'text-[#FF4757]', label: 'Rejected' },
    revision: { bg: 'bg-[#2F55FF]/20', text: 'text-[#2F55FF]', label: 'Revision Requested' }
  };
    ...
    <div className="space-y-6 animate-fade-in" data-testid="brand-approvals">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/50 text-sm">Review and approve campaign deliverables</p>
        </div>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-4 gap-4">
        <div className="dashboard-card p-4 border-l-4 border-[#FFA502]">
          <p className="text-white/50 text-sm">Pending Review</p>
          <p className="text-2xl font-bold text-[#FFA502]">{approvals.filter(a => a.status === 'pending').length}</p>
        </div>
        <div className="dashboard-card p-4">
          <p className="text-white/50 text-sm">Revision Requested</p>
          <p className="text-2xl font-bold text-[#2F55FF]">{approvals.filter(a => a.status === 'revision').length}</p>
        </div>
        <div className="dashboard-card p-4">
          <p className="text-white/50 text-sm">Approved (MTD)</p>
          <p className="text-2xl font-bold text-[#6BFF9A]">{approvals.filter(a => a.status === 'approved').length}</p>
        </div>
        <div className="dashboard-card p-4">
          <p className="text-white/50 text-sm">Avg Review Time</p>
          <p className="text-2xl font-bold text-white">1.8 days</p>
        </div>
      </div>
        ...
        {filtered.map((item) => {
          ...
          return (
            <div key={item.id} className="dashboard-card p-6" data-testid={`approval-${item.id}`}>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      ...
                      <span className="text-white/30 text-xs">{item.id}</span>
                    </div>
                    <h3 className="text-white font-semibold">{item.title}</h3>
                    <p className="text-white/50 text-sm">{item.campaign}</p>
                    <p className="text-white/70 text-sm mt-2">{item.description}</p>
                  </div>
                </div>
                <div className="text-right text-sm">
                  <p className="text-white/40">Submitted: {item.submitted}</p>
                  <p className="text-white/40">Deadline: {item.deadline}</p>
                </div>
              </div>
              ...
              {item.revision_notes && (
                <div className="mb-4 bg-[#2F55FF]/10 border border-[#2F55FF]/20 rounded-lg p-3">
                  <p className="text-[#2F55FF] text-xs mb-1">Revision Notes</p>
                  <p className="text-white/80 text-sm">{item.revision_notes}</p>
                </div>
              )}
              ...
              <div className="flex items-center justify-end gap-2 pt-4 border-t border-white/5">
                ...
                {item.status === 'pending' && (
                  <>
                    <button className="text-sm px-4 py-2 rounded-lg bg-[#FF4757]/20 text-[#FF4757] hover:bg-[#FF4757]/30 flex items-center gap-1">
