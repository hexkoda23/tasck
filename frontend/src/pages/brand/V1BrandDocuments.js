...
import { DocumentIcon, ErrorState, LoadingState, SnapshotSections, approveDocument, brandContact, cleanPortalText, documentGroupsFromBundles, formatDate, latestRevisionInfo, sectionsFromSnapshot, sendDocumentComment, sendReportFeedback, sentenceCaseStatus, snapshotRevision, statusTone, useV1BrandPortalData } from './V1BrandPortalData';
...
import { PitchDeckFlipbook } from '../../components/v1/PitchDeckFlipbook';
...
const DocumentReview = ({ kind, title, emptyLabel }) => {
  ...
  const snapshotTitle = (selected?.businessCase?.title || selected?.snapshot?.title || 'TASCK document') + (isRevised ? ` (Rev ${revision})` : '');
