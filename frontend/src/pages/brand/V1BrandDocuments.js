...
import { v3AlignmentDocxUrl, v3ContractDocxUrl, v3FinalReportPdfUrl, v3StrategySnapshotDocxUrl } from '../../lib/v3api';

const downloadUrlFor = (doc) => { if (doc.kind === 'alignment' && doc.snapshot?.id) return v3AlignmentDocxUrl(doc.snapshot.id); if (doc.kind === 'strategy' && doc.snapshot?.id) return v3StrategySnapshotDocxUrl(doc.snapshot.id); if (doc.kind === 'contract' && doc.snapshot?.id) return v3ContractDocxUrl(doc.snapshot.id); if (doc.kind === 'report' && doc.snapshot?.id) return v3FinalReportPdfUrl(doc.snapshot.id); return ''; };
