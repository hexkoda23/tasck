      ...
      <div className="dashboard-card p-5 bg-gradient-to-br from-[#2F55FF]/20 to-[#6BFF9A]/10">
        <div className="flex items-start justify-between mb-6">
          <div>
            ...
            <p className="text-4xl font-bold text-[#0F172A] font-mono">{formatNaira(user?.wallet_balance || 2875000)}</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="bg-[#F8FAFC] rounded-lg p-3">
            <p className="text-[#94A3B8] text-xs mb-1">Pending Earnings</p>
            <p className="text-xl font-mono text-[#D97706]">₦562,500</p>
          </div>
          <div className="bg-[#F8FAFC] rounded-lg p-3">
            <p className="text-[#94A3B8] text-xs mb-1">Total Earned (YTD)</p>
            <p className="text-xl font-mono text-[#0F172A]">₦4,650,000</p>
          </div>
        </div>

        <div className="flex gap-3">
          <button className="btn-primary flex-1">Withdraw</button>
          <button className="btn-secondary flex-1 border-white/20 text-[#0F172A]">Add Bank Account</button>
        </div>
      </div>
      ...
      <div className="dashboard-card p-5">
        ...
        <div className="flex items-center justify-between p-4 bg-[#F8FAFC] rounded-lg">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[#0F172A] font-medium">Zenith Bank</p>
              <p className="text-[#94A3B8] text-sm">****7892 • {user?.name || 'Kelechi Amadi-Obi'}</p>
            </div>
          </div>
            ...
            <div key={tx.id} className="flex items-center justify-between py-3 border-b border-[#F1F5F9] last:border-0">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                  tx.type === 'credit' ? 'bg-[#22C55E]/20' : 'bg-[#FEF2F2]'
                }`}>
                  {tx.type === 'credit' ? (
              ...
              <div className="text-right">
                <p className={`font-mono ${tx.type === 'credit' ? 'text-[#22C55E]' : 'text-[#DC2626]'}`}>
